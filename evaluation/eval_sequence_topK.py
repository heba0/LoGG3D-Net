...
def evaluate_sequence_topK(model, cfg):
    logging.info(f'Evaluating TOPK')
    ...
    test_topK = np.arange(1, 6)
    
    num_true_positive = np.zeros((len(test_topK), num_thresholds))
    num_false_positive = np.zeros((len(test_topK), num_thresholds))
    num_true_negative = np.zeros((len(test_topK), num_thresholds))
    num_false_negative = np.zeros((len(test_topK), num_thresholds))
    ...
    for i in range(len(test_topK)):
        ...
        for query_idx in range(num_queries):
            ...
            prep_timer.toc()
            desc_timer.tic()
            output_desc, output_feats = model(input, topK=i + 1)  # .squeeze()
            desc_timer.toc()
            ...
            for thres_idx in range(num_thresholds):
                ...
                if(min_dist < threshold):  # Positive Prediction
                    if p_dist <= cfg.revisit_criteria:
                        num_true_positive[i][thres_idx] += 1

                    elif p_dist > cfg.not_revisit_criteria:
                        num_false_positive[i][thres_idx] += 1

                else:  # Negative Prediction
                    if(is_revisit == 0):
                        num_true_negative[i][thres_idx] += 1
                    else:
                        num_false_negative[i][thres_idx] += 1

    F1max = 0.0
    Precisions, Recalls = [], []
    Plot_Precisions, Plot_Recalls = [], []
    if not save_descriptors:
        for i in range(len(test_topK)):
            for ithThres in range(num_thresholds):
                nTrueNegative = num_true_negative[i][ithThres]
                nFalsePositive = num_false_positive[i][ithThres]
                nTruePositive = num_true_positive[i][ithThres]
                nFalseNegative = num_false_negative[i][ithThres]
            ...
            Plot_Precisions.append(Precisions)
            Plot_Recalls.append(Recalls)
        ...
        if plot_pr_curve:
            plt.title('Seq: ' + str(eval_seq) +
                      '    F1Max: ' + "%.4f" % (F1max))
            for i in range(len(Plot_Precisions)):
                Recalls = Plot_Recalls[i]
                Precisions = Plot_Precisions[i]
                plt.plot(Recalls, Precisions, marker='.', c=np.random.rand(3,), label='topK = {}'.format(i + 1))
                plt.xlabel('Recall')
