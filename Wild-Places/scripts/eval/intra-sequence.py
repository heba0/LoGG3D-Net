...
def eval_singlesession(database, embeddings, args):
    ...
    embeddings = get_latent_vectors_logg3d(model=model, dataset=database, dataset_folder=args.pcd_path, topK=1, gd_dim=args.logg3d_outdim * args.logg3d_outdim)
    ...
    ## Ploting PR curve
    eval_seq = str(org_database).split('/')[-1].split('.')[-2]
    ...
    plt.savefig(save_dir + '/' + eval_seq + '_' + eval_cpt + '.png')
...
if __name__ == '__main__':
    ...
    for database, embeddings, location in zip(args.databases, args.database_features, args.run_names):
        ...
        stats.loc[location] = [temp_stats['F1max'], temp_stats['Recall@1'], temp_stats['Sequence Length'], temp_stats['Num. Revisits'], temp_stats['Num. Correct Locations'], temp_stats['Model Inference Time']]
    print(stats)
